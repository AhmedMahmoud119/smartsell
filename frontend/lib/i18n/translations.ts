export const translations = {
  ar: {
    landing: {
      title: 'SmartSell',
      subtitle: 'منصة التجارة الإلكترونية المدعومة بالذكاء الاصطناعي للتجار العرب',
      login: 'تسجيل الدخول',
      getStarted: 'ابدأ الآن',
      hero: {
        badge: 'منصة تجارة إلكترونية مدعومة بالذكاء الاصطناعي',
        title: 'أنشئ متجرك الإلكتروني',
        titleHighlight: 'في دقائق',
        subtitle: 'ابدأ البيع عبر الإنترنت بسهولة مع SmartSell - منصة شاملة لإدارة متجرك الإلكتروني',
        cta: 'ابدأ مجاناً',
        watchDemo: 'شاهد العرض التوضيحي',
        stats: {
          stores: 'متجر نشط',
          products: 'منتج مباع',
          uptime: 'وقت تشغيل',
        },
      },
      features: {
        title: 'كل ما تحتاجه للنجاح',
        subtitle: 'ميزات قوية مصممة للتجارة الإلكترونية الحديثة',
        speed: {
          title: 'إعداد سريع كالبرق',
          description: 'أطلق متجرك الإلكتروني في دقائق وليس أيام',
        },
        secure: {
          title: 'آمن وموثوق',
          description: 'أمان على مستوى المؤسسات لحماية عملك وعملائك',
        },
        payments: {
          title: 'طرق دفع متعددة',
          description: 'اقبل المدفوعات عبر الدفع عند الاستلام، البطاقات، والمحافظ الرقمية',
        },
        analytics: {
          title: 'تحليلات متقدمة',
          description: 'تتبع المبيعات والعملاء والأداء برؤى تفصيلية',
        },
        mobile: {
          title: 'متوافق مع الجوال',
          description: 'متاجر جميلة تعمل بشكل مثالي على جميع الأجهزة',
        },
        bilingual: {
          title: 'دعم العربية والإنجليزية',
          description: 'دعم كامل للغة العربية وواجهة ثنائية اللغة',
        },
      },
      howItWorks: {
        title: 'كيف يعمل',
        subtitle: 'ابدأ البيع في 4 خطوات بسيطة',
        step1: {
          title: 'أنشئ حساب',
          description: 'سجل في ثوانٍ باستخدام بريدك الإلكتروني',
        },
        step2: {
          title: 'أعد متجرك',
          description: 'خصص متجرك باستخدام منشئنا السهل',
        },
        step3: {
          title: 'أضف المنتجات',
          description: 'ارفع المنتجات وابدأ البيع',
        },
        step4: {
          title: 'ابدأ البيع',
          description: 'شارك متجرك وشاهد الطلبات تتدفق',
        },
      },
      pricing: {
        title: 'خطط تسعير بسيطة',
        subtitle: 'اختر الخطة المناسبة لعملك',
        free: {
          name: 'مجاني',
          price: '0',
          description: 'مثالي للبدء',
          features: ['1 متجر', '50 منتج', '100 طلب شهرياً', 'دعم المجتمع'],
          cta: 'ابدأ مجاناً',
        },
        pro: {
          name: 'احترافي',
          price: '99',
          description: 'للأعمال المتنامية',
          features: ['متاجر غير محدودة', 'منتجات غير محدودة', 'طلبات غير محدودة', 'دعم أولوية', 'تحليلات متقدمة', 'نطاق مخصص'],
          cta: 'ابدأ تجربة مجانية',
          popular: 'الأكثر شعبية',
        },
      },
      testimonials: {
        title: 'ماذا يقول عملاؤنا',
        subtitle: 'انضم إلى آلاف التجار الذين يثقون بـ SmartSell',
        testimonial1: {
          name: 'أحمد محمد',
          role: 'مالك متجر إلكتروني',
          content: 'SmartSell غيرت طريقة إدارتي لمتجري بالكامل. سهلة الاستخدام وقوية جداً!',
        },
        testimonial2: {
          name: 'سارة جونسون',
          role: 'مديرة تجارة إلكترونية',
          content: 'أفضل منصة استخدمتها لإدارة متجري الإلكتروني. أوصي بها بشدة!',
        },
        testimonial3: {
          name: 'محمد العلي',
          role: 'تاجر تجزئة',
          content: 'الدعم الفني ممتاز والمنصة سهلة جداً. بدأت البيع في أول يوم!',
        },
      },
      faq: {
        title: 'الأسئلة الشائعة',
        subtitle: 'إجابات على الأسئلة الأكثر شيوعاً',
        q1: {
          question: 'هل SmartSell مجاني؟',
          answer: 'نعم، نقدم خطة مجانية تماماً تتيح لك إنشاء متجر واحد مع 50 منتج و100 طلب شهرياً.',
        },
        q2: {
          question: 'ما هي طرق الدفع المدعومة؟',
          answer: 'ندعم الدفع عند الاستلام، البطاقات الائتمانية، التحويل البنكي، والمحافظ الرقمية.',
        },
        q3: {
          question: 'هل يمكنني تخصيص تصميم متجري؟',
          answer: 'بالتأكيد! لديك تحكم كامل في ألوان وتصميم متجرك مع واجهة سهلة الاستخدام.',
        },
        q4: {
          question: 'هل يوجد دعم فني؟',
          answer: 'نعم، نقدم دعم فني عبر البريد الإلكتروني والدردشة المباشرة. العملاء المحترفون يحصلون على دعم أولوية.',
        },
        q5: {
          question: 'هل يمكنني استخدام نطاقي الخاص؟',
          answer: 'نعم، في الخطة الاحترافية يمكنك ربط نطاقك الخاص بمتجرك.',
        },
      },
      cta: {
        title: 'هل أنت مستعد لبدء رحلتك؟',
        subtitle: 'انضم إلى آلاف التجار الذين يبيعون بالفعل مع SmartSell',
        button: 'ابدأ مجاناً →',
      },
      footer: {
        tagline: 'منصة تجارة إلكترونية مدعومة بالذكاء الاصطناعي للتجار العرب',
        rights: '© 2026 SmartSell. جميع الحقوق محفوظة.',
      },
    },
    auth: {
      login: {
        title: 'مرحباً بعودتك',
        subtitle: 'سجل دخولك إلى حساب SmartSell الخاص بك',
        email: 'البريد الإلكتروني',
        password: 'كلمة المرور',
        signIn: 'تسجيل الدخول',
        signingIn: 'جارٍ تسجيل الدخول...',
        noAccount: 'ليس لديك حساب؟',
        signUp: 'سجل الآن',
        continueWithGoogle: 'تابع باستخدام Google',
      },
      register: {
        title: 'إنشاء حساب',
        subtitle: 'ابدأ مع SmartSell مجاناً',
        fullName: 'الاسم الكامل',
        email: 'البريد الإلكتروني',
        password: 'كلمة المرور',
        passwordHint: '8 أحرف على الأقل',
        workspaceName: 'اسم مساحة العمل',
        workspaceHint: 'يمكنك إنشاء متاجر متعددة في مساحة عمل واحدة',
        createAccount: 'إنشاء حساب',
        creatingAccount: 'جارٍ إنشاء الحساب...',
        haveAccount: 'لديك حساب بالفعل؟',
        signIn: 'سجل الدخول',
        continueWithGoogle: 'تابع باستخدام Google',
      },
    },
    dashboard: {
      welcomeBack: 'مرحباً بعودتك، {name}!',
      subtitle: 'إليك ما يحدث مع متاجرك اليوم.',
      workspace: 'مساحة العمل',
      dashboard: 'لوحة التحكم',
      stores: 'المتاجر',
      settings: 'الإعدادات',
      profile: 'الملف الشخصي',
      logout: 'تسجيل الخروج',
      totalStores: 'إجمالي المتاجر',
      totalProducts: 'إجمالي المنتجات',
      totalOrders: 'إجمالي الطلبات',
      noStores: 'لم يتم إنشاء متاجر بعد',
      noProducts: 'لم تتم إضافة منتجات بعد',
      noOrders: 'لم يتم استلام طلبات بعد',
    },
    analytics: {
      title: 'التحليلات',
      overview: 'نظرة عامة',
      revenue: 'الإيرادات',
      orders: 'الطلبات',
      sales: 'المبيعات',
      revenueOverTime: 'الإيرادات خلال الفترة',
      ordersByStatus: 'الطلبات حسب الحالة',
      topProducts: 'أفضل المنتجات مبيعاً',
      topCustomers: 'أفضل العملاء',
      salesByStore: 'المبيعات حسب المتجر',
      recentOrders: 'أحدث الطلبات',
      totalRevenue: 'إجمالي الإيرادات',
      averageOrder: 'متوسط الطلب',
      conversionRate: 'معدل التحويل',
      product: 'المنتج',
      store: 'المتجر',
      ordersCount: 'عدد الطلبات',
      noData: 'لا توجد بيانات للعرض',
      noProducts: 'لا توجد منتجات',
      last7Days: 'آخر 7 أيام',
      last30Days: 'آخر 30 يوم',
      last90Days: 'آخر 90 يوم',
      lastYear: 'آخر سنة',
      growth: 'نمو',
      decline: 'انخفاض',
      viewAll: 'عرض الكل',
    },
    stores: {
      title: 'متاجري',
      createStore: 'إنشاء متجر جديد',
      noStoresYet: 'لا توجد متاجر حتى الآن',
      createFirstStore: 'إنشاء متجرك الأول',
      storeName: 'اسم المتجر',
      description: 'الوصف',
      language: 'اللغة',
      currency: 'العملة',
      creating: 'جارٍ الإنشاء...',
      create: 'إنشاء',
      cancel: 'إلغاء',
      edit: 'تعديل',
      delete: 'حذف',
      view: 'عرض',
      products: 'منتجات',
      orders: 'طلبات',
      status: 'الحالة',
      draft: 'مسودة',
      published: 'منشور',
      paused: 'متوقف',
      archived: 'مؤرشف',
      settings: 'إعدادات المتجر',
      basicInfo: 'المعلومات الأساسية',
      contactInfo: 'معلومات الاتصال',
      policies: 'السياسات',
      seo: 'تحسين محركات البحث',
      email: 'البريد الإلكتروني',
      phone: 'رقم الهاتف',
      whatsapp: 'واتساب',
      address: 'العنوان',
      shippingPolicy: 'سياسة الشحن',
      returnPolicy: 'سياسة الإرجاع',
      privacyPolicy: 'سياسة الخصوصية',
      termsOfService: 'شروط الخدمة',
      metaTitle: 'عنوان الصفحة',
      metaDescription: 'وصف الصفحة',
      save: 'حفظ',
      saving: 'جارٍ الحفظ...',
      saved: 'تم الحفظ',
      deleteConfirm: 'هل أنت متأكد من حذف هذا المتجر؟',
      deleteWarning: 'سيتم حذف جميع المنتجات والطلبات المرتبطة بهذا المتجر.',
    },
    products: {
      title: 'المنتجات',
      addProduct: 'إضافة منتج',
      noProductsYet: 'لا توجد منتجات حتى الآن',
      createFirstProduct: 'أضف منتجك الأول',
      productName: 'اسم المنتج',
      price: 'السعر',
      compareAtPrice: 'سعر المقارنة',
      costPrice: 'سعر التكلفة',
      sku: 'رمز SKU',
      stock: 'المخزون',
      trackInventory: 'تتبع المخزون',
      images: 'الصور',
      video: 'فيديو',
      creating: 'جارٍ الإنشاء...',
      saving: 'جارٍ الحفظ...',
      selectStore: 'اختر المتجر',
      inStock: 'متوفر',
      outOfStock: 'نفذ المخزون',
      lowStock: 'مخزون منخفض',
      active: 'نشط',
      inactive: 'غير نشط',
      deleteConfirm: 'هل أنت متأكد من حذف هذا المنتج؟',
      bulkActions: 'إجراءات جماعية',
      selected: 'محدد',
      makeActive: 'تفعيل',
      makeDraft: 'تحويل لمسودة',
      makeArchived: 'أرشفة',
      productDetails: 'تفاصيل المنتج',
      pricing: 'التسعير',
      inventory: 'المخزون',
      media: 'الوسائط',
      allStores: 'جميع المتاجر',
      // Discount
      discount: 'الخصم',
      enableDiscount: 'تفعيل الخصم',
      discountType: 'نوع الخصم',
      percentage: 'نسبة مئوية (%)',
      fixedAmount: 'مبلغ ثابت',
      percentageValue: 'النسبة',
      amountOff: 'المبلغ المخصوم',
      originalPrice: 'السعر الأصلي',
      discountAmount: 'الخصم',
      finalPrice: 'السعر النهائي',
      // Variants
      variants: 'المتغيرات',
      variantColor: 'لون',
      variantSize: 'مقاس',
      variantText: 'نص',
      selectColors: 'اختر الألوان',
      selectSizes: 'اختر المقاسات',
      addCustomVariant: 'إضافة متغير مخصص',
      variantsList: 'قائمة المتغيرات',
      variantName: 'اسم المتغير',
      totalStock: 'إجمالي المخزون',
      // Image Upload
      dropImages: 'اسحب الصور هنا أو انقر للتحميل',
      imageFormats: 'JPG, PNG, WEBP, GIF (حد أقصى 5 ميجا)',
      maxImagesReached: 'وصلت للحد الأقصى من الصور',
      mainImage: 'الرئيسية',
      imagesUploaded: 'صور تم تحميلها',
    },
    orders: {
      title: 'الطلبات',
      orderNumber: 'رقم الطلب',
      customer: 'العميل',
      status: 'الحالة',
      total: 'الإجمالي',
      date: 'التاريخ',
      noOrdersYet: 'لا توجد طلبات حتى الآن',
      pending: 'قيد الانتظار',
      processing: 'قيد المعالجة',
      completed: 'مكتمل',
      canceled: 'ملغي',
      paymentStatus: 'حالة الدفع',
      paymentPending: 'في انتظار الدفع',
      paymentPaid: 'مدفوع',
      paymentFailed: 'فشل الدفع',
      fulfillmentStatus: 'حالة التنفيذ',
      unfulfilled: 'غير منفذ',
      shipped: 'تم الشحن',
      delivered: 'تم التسليم',
      customerDetails: 'بيانات العميل',
      customerName: 'اسم العميل',
      customerEmail: 'البريد الإلكتروني',
      customerPhone: 'رقم الهاتف',
      shippingAddress: 'عنوان الشحن',
      orderItems: 'عناصر الطلب',
      product: 'المنتج',
      quantity: 'الكمية',
      price: 'السعر',
      subtotal: 'المجموع الفرعي',
      shipping: 'الشحن',
      tax: 'الضريبة',
      discount: 'الخصم',
      paymentMethod: 'طريقة الدفع',
      cod: 'الدفع عند الاستلام',
      card: 'بطاقة ائتمانية',
      trackingNumber: 'رقم التتبع',
      carrier: 'شركة الشحن',
      notes: 'ملاحظات',
      cancelReason: 'سبب الإلغاء',
      updateStatus: 'تحديث الحالة',
      markAsPaid: 'تحديد كمدفوع',
      markAsShipped: 'تحديد كمشحون',
      markAsDelivered: 'تحديد كمسلم',
      cancelOrder: 'إلغاء الطلب',
    },
    customers: {
      title: 'العملاء',
      addCustomer: 'إضافة عميل',
      noCustomersYet: 'لا يوجد عملاء حتى الآن',
      createFirstCustomer: 'أضف عميلك الأول',
      customerName: 'اسم العميل',
      email: 'البريد الإلكتروني',
      phone: 'رقم الهاتف',
      address: 'العنوان',
      city: 'المدينة',
      state: 'المنطقة',
      zipCode: 'الرمز البريدي',
      country: 'الدولة',
      totalOrders: 'إجمالي الطلبات',
      totalSpent: 'إجمالي المبلغ المنفق',
      acceptsMarketing: 'يقبل التسويق',
      notes: 'ملاحظات',
      creating: 'جارٍ الإنشاء...',
      saving: 'جارٍ الحفظ...',
      selectStore: 'اختر المتجر',
      deleteConfirm: 'هل أنت متأكد من حذف هذا العميل؟',
      customerDetails: 'تفاصيل العميل',
      recentOrders: 'الطلبات الأخيرة',
      orderHistory: 'سجل الطلبات',
    },
    pixels: {
      title: 'أكواد التتبع',
      description: 'إدارة أكواد التتبع والتسويق لمتجرك',
      pixelId: 'معرف البكسل',
      accessToken: 'رمز الوصول',
      accessTokenHelp: 'مطلوب لـ Conversion API (تتبع من الخادم)',
      testMode: 'وضع الاختبار',
      testPixel: 'اختبار البكسل',
      testSuccess: 'البكسل يعمل بشكل صحيح',
      testFailed: 'فشل اختبار البكسل',
      facebookDescription: 'تتبع زوار Facebook وتحويلاتهم',
      tiktokDescription: 'تتبع زوار TikTok وتحويلاتهم',
      gtmDescription: 'إدارة جميع أكواد التتبع من مكان واحد',
      clarityDescription: 'تسجيلات الجلسات وخرائط الحرارة',
      containerId: 'معرف الحاوية',
      projectId: 'معرف المشروع',
      customScripts: 'أكواد مخصصة',
      customScriptsHelp: 'أضف أكواد تتبع إضافية (JavaScript)',
      headScripts: 'أكواد الرأس (Head)',
      bodyScripts: 'أكواد الجسم (Body)',
    },
    settings: {
      title: 'الإعدادات',
      description: 'إدارة إعدادات وتفضيلات مساحة العمل',
      general: 'عام',
      billing: 'الفواتير',
      comingSoon: 'قريباً...',
      currencies: 'العملات',
      manageCurrencies: 'إدارة العملات',
      manageCurrenciesDescription: 'إضافة أو تعديل أو حذف العملات لمساحة العمل',
      addCurrency: 'إضافة عملة',
      editCurrency: 'تعديل العملة',
      addFirstCurrency: 'أضف أول عملة',
      noCurrencies: 'لا توجد عملات بعد',
      confirmDeleteCurrency: 'هل أنت متأكد من حذف هذه العملة؟',
      currencyCode: 'رمز العملة',
      currencyName: 'اسم العملة',
      currencyNameAr: 'اسم العملة (عربي)',
      currencySymbol: 'رمز العملة',
      activeCurrency: 'نشط',
      inactive: 'غير نشط',
      currenciesDescription: 'إدارة العملات المتاحة في متجرك',
      availableCurrencies: 'العملات المتاحة',
      currenciesInfo: 'هذه جميع العملات المتاحة في النظام. يمكنك تفعيل عملات محددة لكل متجر.',
      storeSpecificCurrencies: 'عملات خاصة بالمتجر',
      storeSpecificCurrenciesInfo: 'لتفعيل عملات محددة لمتجر، انتقل إلى إعدادات المتجر ← العملات لكل متجر على حدة.',
      defaultCurrency: 'العملة الافتراضية',
      defaultCurrencyDescription: 'هذه هي العملة الأساسية لمتجرك. سيتم عرض جميع الأسعار بهذه العملة.',
      enabledCurrencies: 'العملات المفعّلة',
      enabledCurrenciesDescription: 'اختر العملات التي يمكن للعملاء استخدامها لعرض الأسعار.',
      autoConvert: 'تحويل الأسعار تلقائياً',
      autoConvertDescription: 'تحويل الأسعار تلقائياً إلى العملة المختارة من العميل باستخدام أسعار الصرف.',
      default: 'افتراضي',
      selectCurrency: 'اختر العملة',
      currency: 'العملة',
      active: 'نشط',
      currencyNamePlaceholder: 'مثال: ريال سعودي',
      currencySymbolPlaceholder: 'مثال: ر.س',
    },
    products: {
      title: 'المنتجات',
      createProduct: 'إنشاء منتج',
      editProduct: 'تعديل المنتج',
      createProductDescription: 'إنشاء منتج جديد لمساحة العمل الخاصة بك',
      addProduct: 'إضافة منتج',
      productDetails: 'تفاصيل المنتج',
      productName: 'اسم المنتج',
      pricing: 'التسعير',
      price: 'السعر',
      compareAtPrice: 'السعر المقارن',
      costPrice: 'سعر التكلفة',
      inventory: 'المخزون',
      sku: 'رمز المنتج (SKU)',
      trackInventory: 'تتبع المخزون',
      stock: 'الكمية',
      media: 'الوسائط',
      images: 'الصور',
      videoUrl: 'رابط الفيديو',
      storeAssignment: 'تعيين المتجر',
      assignToStore: 'تعيين إلى متجر',
      noStore: 'بدون متجر (مستوى مساحة العمل)',
      storeAssignmentHelp: 'اتركه فارغاً لإنشاء منتج على مستوى مساحة العمل. يمكنك تعيينه إلى متجر لاحقاً.',
      allStores: 'جميع المتاجر',
      selected: 'محدد',
      makeActive: 'تفعيل',
      makeDraft: 'جعله مسودة',
      makeArchived: 'أرشفة',
      active: 'نشط',
      inactive: 'غير نشط',
      outOfStock: 'نفذت الكمية',
      lowStock: 'كمية منخفضة',
      inStock: 'متوفر',
      discount: 'الخصم',
      enableDiscount: 'تفعيل الخصم',
      discountType: 'نوع الخصم',
      percentage: 'نسبة مئوية (%)',
      fixedAmount: 'مبلغ ثابت',
      percentageValue: 'النسبة المئوية',
      amountOff: 'قيمة الخصم',
      originalPrice: 'السعر الأصلي',
      discountAmount: 'قيمة الخصم',
      finalPrice: 'السعر النهائي',
      maxImagesReached: 'تم الوصول إلى الحد الأقصى للصور',
      dropImages: 'اسحب الصور هنا أو انقر للتحميل',
      imageFormats: 'JPG, PNG, WEBP, GIF (حد أقصى 5 ميجابايت لكل صورة)',
      mainImage: 'الصورة الرئيسية',
      imagesUploaded: 'صور تم تحميلها',
      variants: 'المتغيرات',
      enableVariants: 'تفعيل المتغيرات',
      variantOptions: 'خيارات المتغيرات',
      addOption: 'إضافة خيار',
      optionName: 'اسم الخيار (مثل: الحجم، اللون)',
      optionValue: 'القيمة (مثل: أحمر، XL)',
      addValue: 'إضافة قيمة',
      generateVariants: 'إنشاء المتغيرات',
      variant: 'المتغير',
      quantityOffers: 'عروض الكمية',
      enableOffers: 'تفعيل العروض',
      offer: 'العرض',
      addOffer: 'إضافة عرض',
      offersInfo: 'سيرى العملاء أسعارًا مخفضة عند الشراء بالكميات المحددة',
      quantity: 'الكمية',
      mainData: 'البيانات الأساسية',
      offers: 'العروض',
    },
    common: {
      loading: 'جارٍ التحميل...',
      save: 'حفظ',
      cancel: 'إلغاء',
      delete: 'حذف',
      edit: 'تعديل',
      create: 'إنشاء',
      close: 'إغلاق',
      yes: 'نعم',
      no: 'لا',
      search: 'بحث',
      filter: 'تصفية',
      actions: 'الإجراءات',
      configure: 'إعداد',
      status: 'الحالة',
      optional: 'اختياري',
      uploading: 'جارٍ التحميل...',
      direction: 'rtl',
    },
  },
  en: {
    landing: {
      title: 'SmartSell',
      subtitle: 'AI-Powered E-Commerce Platform for Arabic Merchants',
      login: 'Login',
      getStarted: 'Get Started',
      hero: {
        badge: 'AI-Powered E-Commerce Platform',
        title: 'Build Your Dream Store',
        titleHighlight: 'In Minutes',
        subtitle: 'Start selling online easily with SmartSell - a complete platform for managing your e-commerce store',
        cta: 'Start Free',
        watchDemo: 'Watch Demo',
        stats: {
          stores: 'Active Stores',
          products: 'Products Sold',
          uptime: 'Uptime',
        },
      },
      features: {
        title: 'Everything You Need to Succeed',
        subtitle: 'Powerful features built for modern e-commerce',
        speed: {
          title: 'Lightning Fast Setup',
          description: 'Get your online store up and running in minutes, not days',
        },
        secure: {
          title: 'Secure & Reliable',
          description: 'Enterprise-grade security to protect your business and customers',
        },
        payments: {
          title: 'Multiple Payment Methods',
          description: 'Accept payments through COD, cards, and digital wallets',
        },
        analytics: {
          title: 'Advanced Analytics',
          description: 'Track sales, customers, and performance with detailed insights',
        },
        mobile: {
          title: 'Mobile Responsive',
          description: 'Beautiful stores that work perfectly on all devices',
        },
        bilingual: {
          title: 'Arabic & English Support',
          description: 'Full RTL support and bilingual interface out of the box',
        },
      },
      howItWorks: {
        title: 'How It Works',
        subtitle: 'Start selling in 4 simple steps',
        step1: {
          title: 'Create Account',
          description: 'Sign up in seconds with your email',
        },
        step2: {
          title: 'Setup Store',
          description: 'Customize your store with our easy builder',
        },
        step3: {
          title: 'Add Products',
          description: 'Upload products and start selling',
        },
        step4: {
          title: 'Start Selling',
          description: 'Share your store and watch orders come in',
        },
      },
      pricing: {
        title: 'Simple Pricing Plans',
        subtitle: 'Choose the right plan for your business',
        free: {
          name: 'Free',
          price: '0',
          description: 'Perfect for getting started',
          features: ['1 Store', '50 Products', '100 Orders/month', 'Community Support'],
          cta: 'Start Free',
        },
        pro: {
          name: 'Professional',
          price: '99',
          description: 'For growing businesses',
          features: ['Unlimited Stores', 'Unlimited Products', 'Unlimited Orders', 'Priority Support', 'Advanced Analytics', 'Custom Domain'],
          cta: 'Start Free Trial',
          popular: 'Most Popular',
        },
      },
      testimonials: {
        title: 'What Our Customers Say',
        subtitle: 'Join thousands of merchants who trust SmartSell',
        testimonial1: {
          name: 'Ahmed Mohamed',
          role: 'E-Commerce Store Owner',
          content: 'SmartSell completely transformed how I manage my store. Easy to use and incredibly powerful!',
        },
        testimonial2: {
          name: 'Sarah Johnson',
          role: 'E-Commerce Manager',
          content: 'The best platform I\'ve used for managing my online store. Highly recommended!',
        },
        testimonial3: {
          name: 'Mohamed Alali',
          role: 'Retail Merchant',
          content: 'Excellent technical support and the platform is very easy. I started selling on day one!',
        },
      },
      faq: {
        title: 'Frequently Asked Questions',
        subtitle: 'Answers to the most common questions',
        q1: {
          question: 'Is SmartSell free?',
          answer: 'Yes, we offer a completely free plan that allows you to create 1 store with 50 products and 100 orders per month.',
        },
        q2: {
          question: 'What payment methods are supported?',
          answer: 'We support Cash on Delivery (COD), credit cards, bank transfers, and digital wallets.',
        },
        q3: {
          question: 'Can I customize my store design?',
          answer: 'Absolutely! You have full control over your store colors and design with an easy-to-use interface.',
        },
        q4: {
          question: 'Is there technical support?',
          answer: 'Yes, we provide technical support via email and live chat. Professional customers get priority support.',
        },
        q5: {
          question: 'Can I use my own domain?',
          answer: 'Yes, on the Professional plan you can connect your own custom domain to your store.',
        },
      },
      cta: {
        title: 'Ready to Start Your Journey?',
        subtitle: 'Join thousands of merchants already selling with SmartSell',
        button: 'Get Started for Free →',
      },
      footer: {
        tagline: 'AI-Powered E-Commerce Platform for Arabic Merchants',
        rights: '© 2026 SmartSell. All rights reserved.',
      },
    },
    auth: {
      login: {
        title: 'Welcome Back',
        subtitle: 'Sign in to your SmartSell account',
        email: 'Email',
        password: 'Password',
        signIn: 'Sign In',
        signingIn: 'Signing in...',
        noAccount: "Don't have an account?",
        signUp: 'Sign up',
        continueWithGoogle: 'Continue with Google',
      },
      register: {
        title: 'Create Account',
        subtitle: 'Get started with SmartSell for free',
        fullName: 'Full Name',
        email: 'Email',
        password: 'Password',
        passwordHint: 'At least 8 characters',
        workspaceName: 'Workspace Name',
        workspaceHint: 'You can create multiple stores in a workspace',
        createAccount: 'Create Account',
        creatingAccount: 'Creating account...',
        haveAccount: 'Already have an account?',
        signIn: 'Sign in',
        continueWithGoogle: 'Continue with Google',
      },
    },
    dashboard: {
      welcomeBack: 'Welcome back, {name}!',
      subtitle: "Here's what's happening with your stores today.",
      workspace: 'Workspace',
      dashboard: 'Dashboard',
      stores: 'Stores',
      settings: 'Settings',
      profile: 'Profile',
      logout: 'Logout',
      totalStores: 'Total Stores',
      totalProducts: 'Total Products',
      totalOrders: 'Total Orders',
      noStores: 'No stores created yet',
      noProducts: 'No products added yet',
      noOrders: 'No orders received yet',
    },
    analytics: {
      title: 'Analytics',
      overview: 'Overview',
      revenue: 'Revenue',
      orders: 'Orders',
      sales: 'Sales',
      revenueOverTime: 'Revenue Over Time',
      ordersByStatus: 'Orders by Status',
      topProducts: 'Top Selling Products',
      topCustomers: 'Top Customers',
      salesByStore: 'Sales by Store',
      recentOrders: 'Recent Orders',
      totalRevenue: 'Total Revenue',
      averageOrder: 'Average Order',
      conversionRate: 'Conversion Rate',
      product: 'Product',
      store: 'Store',
      ordersCount: 'Orders',
      noData: 'No data to display',
      noProducts: 'No products',
      last7Days: 'Last 7 days',
      last30Days: 'Last 30 days',
      last90Days: 'Last 90 days',
      lastYear: 'Last year',
      growth: 'growth',
      decline: 'decline',
      viewAll: 'View All',
    },
    stores: {
      title: 'My Stores',
      createStore: 'Create New Store',
      noStoresYet: 'No stores yet',
      createFirstStore: 'Create your first store',
      storeName: 'Store Name',
      description: 'Description',
      language: 'Language',
      currency: 'Currency',
      creating: 'Creating...',
      create: 'Create',
      cancel: 'Cancel',
      edit: 'Edit',
      delete: 'Delete',
      view: 'View',
      products: 'Products',
      orders: 'Orders',
      status: 'Status',
      draft: 'Draft',
      published: 'Published',
      paused: 'Paused',
      archived: 'Archived',
      settings: 'Store Settings',
      basicInfo: 'Basic Information',
      contactInfo: 'Contact Information',
      policies: 'Policies',
      seo: 'SEO',
      email: 'Email',
      phone: 'Phone Number',
      whatsapp: 'WhatsApp',
      address: 'Address',
      shippingPolicy: 'Shipping Policy',
      returnPolicy: 'Return Policy',
      privacyPolicy: 'Privacy Policy',
      termsOfService: 'Terms of Service',
      metaTitle: 'Page Title',
      metaDescription: 'Page Description',
      save: 'Save',
      saving: 'Saving...',
      saved: 'Saved',
      deleteConfirm: 'Are you sure you want to delete this store?',
      deleteWarning: 'All products and orders associated with this store will be deleted.',
    },
    products: {
      title: 'Products',
      addProduct: 'Add Product',
      noProductsYet: 'No products yet',
      createFirstProduct: 'Add your first product',
      productName: 'Product Name',
      price: 'Price',
      compareAtPrice: 'Compare at Price',
      costPrice: 'Cost Price',
      sku: 'SKU',
      stock: 'Stock',
      trackInventory: 'Track Inventory',
      images: 'Images',
      video: 'Video',
      creating: 'Creating...',
      saving: 'Saving...',
      selectStore: 'Select Store',
      inStock: 'In Stock',
      outOfStock: 'Out of Stock',
      lowStock: 'Low Stock',
      active: 'Active',
      inactive: 'Inactive',
      deleteConfirm: 'Are you sure you want to delete this product?',
      bulkActions: 'Bulk Actions',
      selected: 'Selected',
      makeActive: 'Make Active',
      makeDraft: 'Make Draft',
      makeArchived: 'Archive',
      productDetails: 'Product Details',
      pricing: 'Pricing',
      inventory: 'Inventory',
      media: 'Media',
      allStores: 'All Stores',
      // Discount
      discount: 'Discount',
      enableDiscount: 'Enable discount',
      discountType: 'Discount Type',
      percentage: 'Percentage (%)',
      fixedAmount: 'Fixed Amount',
      percentageValue: 'Percentage',
      amountOff: 'Amount Off',
      originalPrice: 'Original Price',
      discountAmount: 'Discount',
      finalPrice: 'Final Price',
      // Variants
      variants: 'Variants',
      variantColor: 'Color',
      variantSize: 'Size',
      variantText: 'Text',
      selectColors: 'Select Colors',
      selectSizes: 'Select Sizes',
      addCustomVariant: 'Add Custom Variant',
      variantsList: 'Variants',
      variantName: 'Variant name',
      totalStock: 'Total Stock',
      // Image Upload
      dropImages: 'Drop images here or click to upload',
      imageFormats: 'JPG, PNG, WEBP, GIF (max 5MB each)',
      maxImagesReached: 'Maximum images reached',
      mainImage: 'Main',
      imagesUploaded: 'images uploaded',
    },
    orders: {
      title: 'Orders',
      orderNumber: 'Order Number',
      customer: 'Customer',
      status: 'Status',
      total: 'Total',
      date: 'Date',
      noOrdersYet: 'No orders yet',
      pending: 'Pending',
      processing: 'Processing',
      completed: 'Completed',
      canceled: 'Canceled',
      paymentStatus: 'Payment Status',
      paymentPending: 'Payment Pending',
      paymentPaid: 'Paid',
      paymentFailed: 'Payment Failed',
      fulfillmentStatus: 'Fulfillment Status',
      unfulfilled: 'Unfulfilled',
      shipped: 'Shipped',
      delivered: 'Delivered',
      customerDetails: 'Customer Details',
      customerName: 'Customer Name',
      customerEmail: 'Email',
      customerPhone: 'Phone Number',
      shippingAddress: 'Shipping Address',
      orderItems: 'Order Items',
      product: 'Product',
      quantity: 'Quantity',
      price: 'Price',
      subtotal: 'Subtotal',
      shipping: 'Shipping',
      tax: 'Tax',
      discount: 'Discount',
      paymentMethod: 'Payment Method',
      cod: 'Cash on Delivery',
      card: 'Credit Card',
      trackingNumber: 'Tracking Number',
      carrier: 'Carrier',
      notes: 'Notes',
      cancelReason: 'Cancel Reason',
      updateStatus: 'Update Status',
      markAsPaid: 'Mark as Paid',
      markAsShipped: 'Mark as Shipped',
      markAsDelivered: 'Mark as Delivered',
      cancelOrder: 'Cancel Order',
    },
    customers: {
      title: 'Customers',
      addCustomer: 'Add Customer',
      noCustomersYet: 'No customers yet',
      createFirstCustomer: 'Add your first customer',
      customerName: 'Customer Name',
      email: 'Email',
      phone: 'Phone',
      address: 'Address',
      city: 'City',
      state: 'State',
      zipCode: 'Zip Code',
      country: 'Country',
      totalOrders: 'Total Orders',
      totalSpent: 'Total Spent',
      acceptsMarketing: 'Accepts Marketing',
      notes: 'Notes',
      creating: 'Creating...',
      saving: 'Saving...',
      selectStore: 'Select Store',
      deleteConfirm: 'Are you sure you want to delete this customer?',
      customerDetails: 'Customer Details',
      recentOrders: 'Recent Orders',
      orderHistory: 'Order History',
    },
    pixels: {
      title: 'Marketing Pixels',
      description: 'Manage tracking and marketing pixels for your store',
      pixelId: 'Pixel ID',
      accessToken: 'Access Token',
      accessTokenHelp: 'Required for Conversion API (server-side tracking)',
      testMode: 'Test Mode',
      testPixel: 'Test Pixel',
      testSuccess: 'Pixel is working correctly',
      testFailed: 'Pixel test failed',
      facebookDescription: 'Track Facebook visitors and conversions',
      tiktokDescription: 'Track TikTok visitors and conversions',
      gtmDescription: 'Manage all tracking codes from one place',
      clarityDescription: 'Session recordings and heatmaps',
      containerId: 'Container ID',
      projectId: 'Project ID',
      customScripts: 'Custom Scripts',
      customScriptsHelp: 'Add additional tracking scripts (JavaScript)',
      headScripts: 'Head Scripts',
      bodyScripts: 'Body Scripts',
    },
    settings: {
      title: 'Settings',
      description: 'Manage your workspace settings and preferences',
      general: 'General',
      billing: 'Billing',
      comingSoon: 'Coming soon...',
      currencies: 'Currencies',
      manageCurrencies: 'Manage Currencies',
      manageCurrenciesDescription: 'Add, edit, or remove currencies for your workspace',
      addCurrency: 'Add Currency',
      editCurrency: 'Edit Currency',
      addFirstCurrency: 'Add your first currency',
      noCurrencies: 'No currencies yet',
      confirmDeleteCurrency: 'Are you sure you want to delete this currency?',
      currencyCode: 'Currency Code',
      currencyName: 'Currency Name',
      currencyNameAr: 'Currency Name (Arabic)',
      currencySymbol: 'Currency Symbol',
      activeCurrency: 'Active',
      inactive: 'Inactive',
      currenciesDescription: 'Manage currencies available in your store',
      availableCurrencies: 'Available Currencies',
      currenciesInfo: 'These are all the currencies available in the system. You can enable specific currencies for each store.',
      storeSpecificCurrencies: 'Store-Specific Currencies',
      storeSpecificCurrenciesInfo: 'To enable specific currencies for a store, go to Store Settings → Currencies for each individual store.',
      defaultCurrency: 'Default Currency',
      defaultCurrencyDescription: 'This is the primary currency for your store. All prices will be displayed in this currency.',
      enabledCurrencies: 'Enabled Currencies',
      enabledCurrenciesDescription: 'Select which currencies customers can use to view prices.',
      autoConvert: 'Auto Convert Prices',
      autoConvertDescription: 'Automatically convert prices to customer\'s selected currency using exchange rates.',
      default: 'Default',
      selectCurrency: 'Select Currency',
      currency: 'Currency',
      active: 'Active',
      currencyNamePlaceholder: 'e.g., Saudi Riyal',
      currencySymbolPlaceholder: 'e.g., ر.س',
    },
    products: {
      title: 'Products',
      createProduct: 'Create Product',
      editProduct: 'Edit Product',
      createProductDescription: 'Create a new product for your workspace',
      addProduct: 'Add Product',
      productDetails: 'Product Details',
      productName: 'Product Name',
      pricing: 'Pricing',
      price: 'Price',
      compareAtPrice: 'Compare at Price',
      costPrice: 'Cost Price',
      inventory: 'Inventory',
      sku: 'SKU',
      trackInventory: 'Track Inventory',
      stock: 'Stock',
      media: 'Media',
      images: 'Images',
      videoUrl: 'Video URL',
      storeAssignment: 'Store Assignment',
      assignToStore: 'Assign to Store',
      noStore: 'No Store (Workspace Level)',
      storeAssignmentHelp: 'Leave empty to create a workspace-level product. You can assign it to a store later.',
      allStores: 'All Stores',
      selected: 'selected',
      makeActive: 'Make Active',
      makeDraft: 'Make Draft',
      makeArchived: 'Archive',
      active: 'Active',
      inactive: 'Inactive',
      outOfStock: 'Out of Stock',
      lowStock: 'Low Stock',
      inStock: 'In Stock',
      discount: 'Discount',
      enableDiscount: 'Enable discount',
      discountType: 'Discount Type',
      percentage: 'Percentage (%)',
      fixedAmount: 'Fixed Amount',
      percentageValue: 'Percentage',
      amountOff: 'Amount Off',
      originalPrice: 'Original Price',
      discountAmount: 'Discount',
      finalPrice: 'Final Price',
      maxImagesReached: 'Maximum images reached',
      dropImages: 'Drop images here or click to upload',
      imageFormats: 'JPG, PNG, WEBP, GIF (max 5MB each)',
      mainImage: 'Main',
      imagesUploaded: 'images uploaded',
      variants: 'Variants',
      enableVariants: 'Enable variants',
      variantOptions: 'Variant Options',
      addOption: 'Add Option',
      optionName: 'Option name (e.g., Size, Color)',
      optionValue: 'Value (e.g., Red, XL)',
      addValue: 'Add value',
      generateVariants: 'Generate Variants',
      variant: 'Variant',
      quantityOffers: 'Quantity-Based Offers',
      enableOffers: 'Enable offers',
      offer: 'Offer',
      addOffer: 'Add Offer',
      offersInfo: 'Customers will see discounted prices when buying in specified quantities',
      quantity: 'Quantity',
      mainData: 'Main Data',
      offers: 'Offers',
    },
    common: {
      loading: 'Loading...',
      save: 'Save',
      cancel: 'Cancel',
      delete: 'Delete',
      edit: 'Edit',
      create: 'Create',
      close: 'Close',
      yes: 'Yes',
      no: 'No',
      search: 'Search',
      filter: 'Filter',
      actions: 'Actions',
      configure: 'Configure',
      status: 'Status',
      optional: 'Optional',
      uploading: 'Uploading...',
      direction: 'ltr',
    },
  },
} as const;

export type Locale = keyof typeof translations;
export type TranslationKey = keyof typeof translations.ar;
